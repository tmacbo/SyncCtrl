...
public class ControlPoint implements HTTPRequestListener
{
	...
	private final String TAG = "ControlPoint";
...
//	}
	...
	public synchronized void addDevice(SSDPPacket ssdpPacket)
	{
...
//		}
	 ...
	 * @return
	 */
	public  Device geneDevice(SSDPPacket ssdpPacket)
	{
		if (ssdpPacket.isRootDevice() == false){
	...
	//	log.e("search target = " + target + ", mx = " + mx);
		...
		Log.i("ControlPoint", msReq.toString());
	...
	public boolean subscribe(Service service, long timeout)
	{
		...
		if(ifAddress.equals("")){
//			ifAddress = WifiUtil.getLocalIpAddress();
		}
		
		timeout = 43200;		
		SubscriptionRequest subReq = new SubscriptionRequest();//getEventSubCallbackURL(		
		subReq.setSubscribeRequest(service, ifAddress, timeout);		
		SubscriptionResponse subRes = subReq.post();		
		Log.i("ControlPoint", subRes.toString());
		if (subRes.isSuccessful() == true) {
			service.setSID(subRes.getSID());			
			Log.i("ControlPoint", "当前订阅 获取到的订阅期限为 = "+subRes.getTimeout());
			if(service.getServiceType().equals("urn:schemas-upnp-org:service:AVTransport:1")){
				IrunSinApplication.AVT_SUB_SID = subRes.getSID();//储存订阅号
				Log.i("ControlPoint", "当前订阅 获取到的订阅 AVT_SUB_SID为 = "+subRes.getSID());
			}else if(service.getServiceType().equals("urn:schemas-upnp-org:service:RenderingControl:1")){
				IrunSinApplication.REC_SUB_SID = subRes.getSID();//储存订阅号
				Log.i("ControlPoint", "当前订阅 获取到的订阅 REC_SUB_SID为 = "+subRes.getSID());
			}
	...
	public boolean subscribe(Device device)
	{
		...
		Service avtservice = device.getService("urn:schemas-upnp-org:service:AVTransport:1");
		Service recservice = device.getService("urn:schemas-upnp-org:service:RenderingControl:1");
		Log.i(TAG, "准备开始发送订阅命令");
		boolean avtstate = subscribe(avtservice, Subscription.INFINITE_VALUE);
		if(avtstate){
			Log.i(TAG, "播放服务订阅成功");
			boolean recstate = subscribe(recservice, Subscription.INFINITE_VALUE);
			Log.i(TAG, "第一次控制服务结果 = " + recstate);
			if(!recstate){
				Log.i(TAG, "第一次控制服务订阅失败  发送第二次 ");
				recstate = subscribe(recservice, Subscription.INFINITE_VALUE);
				Log.i(TAG, "第二次控制服务订阅结果 为 = " +recstate);
				if(!recstate){
					return false;
				}
			}
		}else{
			Log.i(TAG, "第一次播放服务订阅失败");
			avtstate = subscribe(avtservice, Subscription.INFINITE_VALUE);
			if(!avtstate){
				Log.i(TAG, "第二次播放服务订阅失败  则报错");
				return false;
			}else{
				Log.i(TAG, "准备发送第一次控制服务订阅");
				boolean recstate = subscribe(recservice, Subscription.INFINITE_VALUE);
				if(!recstate){
					recstate = subscribe(recservice, Subscription.INFINITE_VALUE);
					if(!recstate){
	 ...
	 * @return
	 */
	public void unsubscribe(String sid,Service service){
		SubscriptionRequest subReq = new SubscriptionRequest();
		subReq.setUnsubscribeRequest(service);
		SubscriptionResponse subRes = subReq.post();
	}
